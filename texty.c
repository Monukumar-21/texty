...
#define TEXTY_TAB_STOP 8          // you can change this to any number you like
...
void die(const char *s) {
  // Clear screen and go home before printing error
...
void editorOpen(char *filename) {
  ...
  if (!fp) {
    if (errno == ENOENT) {
      ...
      // The file will be created when the user saves (Ctrl+S).
      return;
    } else {
      die("fopen");
    }
...
void initEditor() {
  ...
  E.screenrows -= 2;   // reserve room for status bar and message bar
}
